...
import { captureDeposit } from '@/lib/deposit-helpers';
import { supabaseServer } from '@/lib/supabaseServer';

// POST /api/deposits/[payment_intent_id]/capture - Capture authorized deposit
export async function POST(request: NextRequest, context: { params: Promise<{ payment_intent_id: string }> }) {
  ...
  const { payment_intent_id } = await context.params;
    ...
    // Normalize type for TypeScript and for the build-time stub
    ...
    if (depositRecord.user_id !== user.id) {
      return NextResponse.json(
        { error: 'Unauthorized to capture this deposit' },
        { status: 403 }
      );
    }
    ...
    // Capture the deposit
    const result = await captureDeposit(payment_intent_id);
